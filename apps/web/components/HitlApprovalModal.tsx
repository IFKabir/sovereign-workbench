...
export default function HitlApprovalModal({ isOpen, onClose, actionType, riskLevel, draftContent, complianceFlags, requiredRole }: HitlModalProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-sovereign-dark/80 backdrop-blur-md">
        ...
        >
          <div className={`p-5 border-b flex justify-between items-center ${riskColors[riskLevel].replace('text-', 'bg-').replace('/10', '/20')} border-b-2`} style={{ borderBottomColor: 'currentcolor' }}>
          ...
          <div className="p-8 space-y-8">
            <div>
              ...
              <p className="font-mono text-gray-200 text-sm bg-sovereign-surface inline-block px-3 py-1.5 rounded border border-sovereign-border">{actionType}</p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-gray-500">Required Role: <span className="font-mono text-accent-cyan px-1.5 py-0.5 rounded bg-accent-cyan/10">{requiredRole}</span></p>
                <p className="text-xs text-gray-500">Block ID pending signature</p>
              </div>
            </div>
          </div>

          <div className="p-5 border-t border-sovereign-border bg-sovereign-surface/80 flex justify-end space-x-4">
            <button onClick={onClose} className="px-5 py-2.5 rounded-lg font-medium border border-sovereign-border hover:bg-sovereign-border/50 hover:text-white transition-colors flex items-center text-gray-400">
              <XCircle className="w-4 h-4 mr-2" /> Reject
            </button>
            <button onClick={onClose} className="px-5 py-2.5 rounded-lg font-medium bg-accent-cyan/20 text-accent-cyan hover:bg-accent-cyan/30 border border-accent-cyan/50 transition-all flex items-center shadow-[0_0_10px_rgba(6,182,212,0.15)] hover:shadow-[0_0_15px_rgba(6,182,212,0.25)]">
